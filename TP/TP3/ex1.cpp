...
Result nearestPoints_BF(std::vector<Point> &vp) {
    Result bestRes = Result();
    for (unsigned int i = 0; i < vp.size(); i++) {
        for (unsigned int j = i + 1; j < vp.size(); j++) {
            if (vp[i].distance(vp[j]) < bestRes.dmin) {
                bestRes.dmin = vp[i].distance(vp[j]);
                bestRes.p1 = vp[i];
                bestRes.p2 = vp[j];
            }
        }
...
Result nearestPoints_BF_SortByX(std::vector<Point> &vp) {
    ...
    for (unsigned int i = 0; i < vp.size(); i++) {
        for (unsigned int j = i + 1; j < vp.size(); j++) {
            if (std::abs(vp[i].x - vp[j].x) > bestRes.dmin) break;
            if (vp[i].distance(vp[j]) < bestRes.dmin) {
                bestRes.dmin = vp[i].distance(vp[j]);
                bestRes.p1 = vp[i];
                bestRes.p2 = vp[j];
...
void nearestPoints_DC_MTRec(std::vector<Point> &vp, int start, int finish, Result& bestResult, int numThreadsAv) {

    //Base case
    if (finish - start <= 2) {
        for (unsigned int i = start; i <= finish; i++) {
            for (unsigned int j = i + 1; j <= finish; j++) {
                if (vp[i].distance(vp[j]) < bestResult.dmin)
                    bestResult = Result(vp[i].distance(vp[j]), vp[i], vp[j]);
            }
        }
        return;
    }
    ...
    //Recursive step
    int finish2 = (start + finish) / 2;
    int start2 = finish2 + 1;
    if (numThreadsAv > 1) {
        std::thread t1([&] () {nearestPoints_DC_MTRec(vp, start2, finish, bestResult, numThreadsAv / 2);});
        //std::thread t2([&] () {nearestPoints_DC_MTRec(vp, start, finish2, bestResult, numThreadsAv / 2);});
        nearestPoints_DC_MTRec(vp, start, finish2, bestResult, numThreadsAv / 2);
        t1.join();
        //t2.join();
    }
    else {
        nearestPoints_DC_MTRec(vp, start2, finish, bestResult, 1);
        nearestPoints_DC_MTRec(vp, start, finish2, bestResult, 1);
    }

    //Search for extra point
    for (unsigned int i = start; i <= finish2; i++) {
        if ((vp[start2].x - vp[i].x) > bestResult.dmin) continue;
        for (unsigned int j = start2; j <= finish; j++) {
            if ((vp[j].x - vp[finish2].x) > bestResult.dmin) break;
            if (vp[i].distance(vp[j]) < bestResult.dmin)
                bestResult = Result(vp[i].distance(vp[j]), vp[i], vp[j]);
        }
    }
...
void nearestPoints_DCRec(std::vector<Point> &vp, int start, int finish, Result& bestResult) {
    ...
    int finish2 = (start + finish) / 2;
    int start2 = finish2 + 1;
    nearestPoints_DCRec(vp, start2, finish, bestResult);
    nearestPoints_DCRec(vp, start, finish2, bestResult);
...
Result nearestPoints_DC(std::vector<Point> &vp) {
    Result res = Result();
    sortByX(vp, 0, vp.size()-1);
    nearestPoints_DCRec(vp, 0, vp.size() - 1, res);
    return res;
}
...
Result nearestPoints_DC_MT(std::vector<Point> &vp) {
    Result res = Result();
    sortByX(vp, 0, vp.size()-1);
    nearestPoints_DC_MTRec(vp, 0, vp.size() - 1, res, numThreads);
    return res;
}
